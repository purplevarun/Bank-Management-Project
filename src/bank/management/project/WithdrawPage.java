...
public class WithdrawPage extends JFrame implements ActionListener {
    ...
    WithdrawPage(String card){
        ...
        //------------------------------------------------------------------------------------------------------    
            ...
            deposit.setBounds(310, 400, 300, 60);
            ...
            if(event.getSource()==deposit){
                if(amt.getText().equals("")){
                    ...
                    try{
                        ...
                        String q0 = "select balance,deposit from statement where cardno='"+tempcard+"'";
                        ResultSet r = con.s.executeQuery(q0);
                        int bal=0,depo=0;
                        while(r.next()){
                            bal = r.getInt("balance");
                            depo = r.getInt("deposit");
                        }
                        if(w>bal){
                            JOptionPane.showMessageDialog(null,"NOT ENOUGH BALANCE !");
                            return;
                        }
                        String q = "insert into statement values('"+tempcard+"','"+(bal-w)+"','"+0+"','"+w+"')";
    ...
    public static void main(String[] args) {
        WithdrawPage x = new WithdrawPage(tempcard);
        x.setVisible(true);
